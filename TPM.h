...
namespace TPM{
    ...
    //  TPM Functions: 
        ...
        void TPM::computeH(){
            /* computeH function: 
                ...
                This function computes the activation levels of the hidden units with the current weights 
            and inputs. The activation levels will be integers between -N*L and N*L, although what is 
            needed for the TPM to compute are just the signs of these activation functions. It might 
            become handy in the future to have this info and from there it is stright forward to get a 
            sign function. 
        ...
        void TPM::compute(){
            /* compute function: 
                ...
                This function performs a whole TPM computation by calling computeH() and computeTau(). 
        ...
        void TPM::updateW(){
            /* updateW function: 
                ...
                This function updates the weights of the TPM. The rule to update the weights is stored 
    ...
    class pTPM: public TPM{
        ...
        public: 
            ...
            int **drawW(); 
            void resetW(), MC_updatePW(), learn_updatePW(); 
    ...
    //  pTPM Functions: 
        ...
        pTPM::pTPM(int nSample_, int nReset_, int K_, int N_, int L_, int updateRule_):TPM(K_, N_, L_, updateRule_){
            /* Constructor function for class pTPM: 
                ...
                This constructor should implement few things more (allocating and init pW) apart from 
            callin super's constructor. 
        ...
        void pTPM::computeSig2W(){
            ...
            */ 
            ...
            for (int i=0; i<getK(); i++){
                for (int j=0; j<getN(); j++){
                    sig2W[i][j]=0.; 
                    for (float k=0; k<2*getL()+1; k++){
                        sig2W[i][j] += pow(k-float(getL())-avW[i][j],2)/(2*getL()+1); // pW[i][j][k]*
                    }
                }
            }
                ...
                for (int j=0; j<getN(); j++){
                    if (updateFactor*getX()[i][j]>0){
                        for (int k=0; k<2*getL(); k++){
                            pW[i][j][k] -= flux*pW[i][j][k]; 
                            pW[i][j][k+1] += flux*pW[i][j][k]; 
                        }
                    }
                    else{
                        for (int k=1; k<2*getL()+1; k++){
                            pW[i][j][k] -= flux*pW[i][j][k]; 
                            pW[i][j][k-1] += flux*pW[i][j][k]; 
                        }
                    }
